...
import { scoresByPos } from './scores';
...
function best(initStones :Stone[][], nextColor :PlayerColor, n :number) :Pos|null{
    ...
    function _scoreFromStones(stones :Stone[][]) :number {
        if (stones.length === 0) {
            ...
            return stones.reduce((ls :number, line :Stone[], y) => {
                return ls + line.reduce((sum, stone, x) => {
                    const scoreAt = scoresByPos[y][x];
                    const ds = stone === nextColor
                        ? scoreAt
                        : stone === Stone.None
                            ? 0
                            : -scoreAt;
